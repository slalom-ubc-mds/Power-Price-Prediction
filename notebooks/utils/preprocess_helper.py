...
def preprocess_intertie_data():
    ...
    try:
        ...
        intertie_df = intertie_df.drop('Date - MST', axis=1)  # Drop the 'Date - MST' column
...
def get_data(start_date, end_date):
    ...
    data = data.drop(columns=['begin_datetime_utc', 'end_datetime_utc', 'end_datetime_mpt'])
    data = data.sort_index()

    # Convert to numeric
    data['system_marginal_price'] = pd.to_numeric(data['system_marginal_price'], errors='coerce')
